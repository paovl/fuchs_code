...
def computeMaxMinBFS(fpath, dataset, fname, info_dataset, rng,  th = 1.5, diskRadius= 45, ransac = 1, px = 100): 
    ...
    for k in range(iterations):
        ...
        if ele and ransac != -1 and (k == 4 or k == 3):
            ...
            if ransac == 1:
                pixels = ransac_bfs(pixels, rng, label, outname + '_' + str(k) + '.png', th = th, diskRadius = diskRadius, state = 1)
...
def dataGenerator(fpath, ipath, hpath, ppath, dataset, fname, info_dataset, rng, th = 1.5, diskRadius = 45, ransac = 1, px = 100):
    ...
    for k in range(iterations):
        ...
        if ele and ransac != -1 and (k == 4 or k == 3):
            ...
            # RANSAC algorithm with sphere fit
            if ransac == 1:
                pixels = ransac_bfs(pixels, rng, label,outname + '_' + str(k) + '.png', th = th, diskRadius = diskRadius)
