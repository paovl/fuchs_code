...
# Fit error for healthy and sick eyes
acc_fit_error_healthy = 0 
n_data_healthy = 1

acc_fit_error_sick = 0 
n_data_sick = 1
...
def computeMaxMinBFS(fpath, dataset, fname, info_dataset, rng,  th = 1.5, diskRadius= 45, ransac = 1, px = 100): 
    """
    ...
    # Get label 
    if outname_split[0] in info_dataset['img_id'].values:
        label = info_dataset['label'][info_dataset['img_id'] == outname_split[0]].values[0]
    else:
        label = -1 
    ...
    for k in range(iterations):
        ...
        if ele and ransac != -1 and (k == 4 or k == 3):
            ...
            if ransac == 1:
                pixels, _ = ransac_bfs(pixels, rng, label, outname + '_' + str(k) + '.png', th = th, diskRadius = diskRadius, state = 1)
...
def dataGenerator(fpath, ipath, hpath, ppath, dataset, fname, info_dataset, rng, th = 1.5, diskRadius = 45, ransac = 1, px = 100):
    ...
    for k in range(iterations):
        ...
        if fname.find('ELE') > 0 and (ransac == -1 or (k != 4 and k != 3)) :
        ...
        if ele and ransac != -1 and (k == 4 or k == 3):
            ...
            # RANSAC algorithm with sphere fit
            if ransac == 1:
                pixels, bfs = ransac_bfs(pixels, rng, label,outname + '_' + str(k) + '.png', th = th, diskRadius = diskRadius)
...
def polarMapGenerator(ipath, ppath, outname, label):
    ...
    img.imsave(os.path.join(polar_path,outname), polar_image[:,::-1,0].astype(np.float64).T,cmap = 'jet')

def ransac_bfs(eleMap, rng, img_label,outname, k=10, th = 0.05, diskRadius= 0, state = 0):
    """
    Params: 
    ...
    plt.figure(4);plt.imshow(eleMap); plt.title('BFS inliers')
    plt.scatter(x_est[idxs_alsoinliers], y_est[idxs_alsoinliers], color='red', s=100, label='Inliers')
    img_in_path = '../datasets/dataset_hryc/SEP/ransac_TH_1.5_r_45/images_in/'

    if img_label == 1: 
        img_in_path = img_in_path + 'SICK/'
    else:
        img_in_path = img_in_path + 'HEALTHY/'

    plt.savefig(img_in_path + outname)
    ...
    if img_label == 0 and state == 0:
        acc_fit_error_healthy += mean_fit_error
        n_data_healthy += 1

    if img_label == 1 and state == 0:
        acc_fit_error_sick += mean_fit_error
        n_data_sick += 1
...
def createIndexFile(fullAnnFile,annFile, data_path, biomarkers=False):
    ...
    columns = ["Ojo","Ojo (OD 0 OI 1)","Descompensación corneal"]
    if biomarkers:
        columns = columns + ["Paqui Relativa","DensitAnt0_2"]
    ...
    # Drop observations with NaN label
    df= df.dropna()
    df["Ojo"] = df["Ojo"].astype(int).astype(str)
    df["Ojo (OD 0 OI 1)"] = df["Ojo (OD 0 OI 1)"].astype(int)
    ...
    new_train_df["img_id"] = df["Ojo"].astype(int).astype(str)
    new_train_df["db"] = "hryc"
    new_train_df["img_path"] = data_path + '/' + df["Ojo"]
    
    new_train_df["hospital"] = "RAMON Y CAJAL"
    new_train_df["lado"] = df["Ojo (OD 0 OI 1)"]
    new_train_df["label"] = df["Descompensación corneal"]
    
    if biomarkers:
        new_train_df["PaqRel"] = df["Paqui Relativa"].str.replace(',','.').astype(float)
        new_train_df["DensAnt"] = df["DensitAnt0_2"].str.replace(',','.').astype(float)
    
    new_train_df.to_csv(annFile,index=False)
                    
def computeNorm(annFile, data_path, px = 100):
    
    dataset = pd.read_csv(annFile,header=0,dtype={'img_id': int, 'label': int})
...
if __name__ == "__main__":
    ...
    if ransac == 1:
        ransac_dir = '/' + eConfig['dir'] + '/ransac_TH_'+ eConfig['th'] + '_r_' + eConfig['r']
    elif ransac == 0: 
        if int(eConfig['r']) > 0:
            ransac_dir = '/' + eConfig['dir'] + '/' + 'noRansac_diskRemoval'
        else:
            ransac_dir = '/' + eConfig['dir'] + '/' + 'noRansac'
    else: 
        ransac_dir = '/' + eConfig['dir'] + '/' + 'original'

    save_path = db_path + ransac_dir
    ...
    # Create index file
    createIndexFile(fullAnnFile,annFile,data_path,biomarkers=False)
    ...
    info_dataset = pd.read_csv(annFile,header=0,dtype={'img_id': str, 'label': int})
    ...
    for f in csv_files:
        ...
        for map_cat in map_cats:
            ...
            if not os.path.exists(csv_path + '/' + csv_name):
                continue

            dataset = readCSV(csv_path,csv_name) # Read data from the csv file
    ...
    for f in csv_files:
        for map_cat in map_cats:
            ...
            if not os.path.exists(csv_path + '/' + csv_name):
                continue
            dataset = readCSV(csv_path,csv_name) # Read data from  csv file
            computeMaxMinBFS(data_path, dataset, name, info_dataset, rng, th = float(eConfig['th']), diskRadius = int(eConfig['r']), ransac = ransac, px = pixels)
    ...
    for f in csv_files:
        ...
        for map_cat in map_cats:
            ...
            print(name)
            dataset = readCSV(csv_path,csv_name) # Read data from csv file
            # We create the input data for CNNs
            dataGenerator( data_path, image_path, heat_path, polar_path, dataset, name, info_dataset, rng, th = float(eConfig['th']), diskRadius = int(eConfig['r']), ransac = ransac, px = pixels)
        
    # Normalization
    cats,mu,sigma = computeNorm(annFile, data_path, px = pixels)
    ...
    # Mean fit error
    mean_fit_error_file = save_path + '/meanELE_error_fit.txt'
    with open(mean_fit_error_file, "w") as file:
        file.write('Mean fit error (Healthy) = ' + str(round(acc_fit_error_healthy/n_data_healthy,4))+ '\n')
        file.write('Mean fit error (Sick) = ' + str(round(acc_fit_error_sick/n_data_sick,4))+ '\n')
