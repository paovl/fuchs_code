...
def computeNorm(annFile, data_path, px = 100):
    ...
    listFiles = glob.glob(os.path.join(data_path,'%s_*.npy'%dataset['img_id'].values[0]));
    categories = ['_'.join(os.path.basename(fichero).split('_')[1:])[:-4] for fichero in listFiles]
...
if __name__ == "__main__":
    ...
    map_cats = ['PAC','CUR','ELE','CORNEA-DENS','ELE_BFS_8mm']
    ...
    for patient, hospital, label in zip(db_index['img_id'], db_index['hospital'], db_index['label']):
        folder = os.path.join(csv_path,hospital,'DATOS EXPORTADOS',patient,'BASAL')
        for cat in map_cats:
            file_pattern = folder + '/*' + cat + '.CSV'
            csv_file = glob.glob(file_pattern)
            if not len(csv_file):
                # bad_patients = bad_patients + ' ' + patient
                print(file_pattern)
                continue

            dataset = readCSV(csv_file[0])
            name = patient + '_' + cat
