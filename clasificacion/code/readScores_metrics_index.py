...
if '__main__':
    ...
    dict_standar_file_bio = 'dict_scores_matrix.pkl'
    ...
    model_dir = ['no_deactivate_seed0_dseed0_th0.07_a1000_sigmoidWeightedLoss(full DB)']

    bio_model_standar_scores_path = f"{bio_data_dir}/{'results_nn_allData'}/{dict_standar_file_bio}"
    ...
    # bio_folds = bio_scores['fold']
    bio_scores = bio_scores['score']

    # Compute mean and std of the scores by fold
    
    bio_scores = (bio_scores - bio_scores.mean()) / bio_scores.std()

    # for fold in range(5):
    #     idx_fold = np.where(bio_folds == fold)[0]
    #     bio_scores_mean = np.mean(bio_scores[idx_fold])
    #     bio_scores_std = np.std(bio_scores[idx_fold])

    #     bio_scores[idx_fold] = (bio_scores[idx_fold] - bio_scores_mean) / bio_scores_std
    ...
    bio_j = bio_tpr + bio_specificity - 1

    bio_j_index = np.argmax(bio_j)
    bio_threshold = bio_thresholds[bio_j_index]

    bio_predict_labels = (bio_scores_norm >= bio_threshold)
    ...
    print("\nBIO THRESHOLD "+ str(bio_threshold)+ "\n")
    print("PRECISION = {}".format(bio_precision))
    print("RECALL = {}".format(bio_recall))
    print("SPECIFITY = {}".format(bio_specifity))
    ...
    for dir in model_dir:
        ...
        predict_labels = (scores_norm >= threshold)
