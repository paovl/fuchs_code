...
if __name__ == '__main__':
    ...
    results_path = results_path + '/' + eConfig['dir_weights_file'] + "_65train"
    if not os.path.exists(results_path):
        os.makedirs(results_path)
    ...
    for iter in np.arange(iterations):
        ...
        # Test dataset
        dataset_idx = random.sample(range(cfg.numFolds), 7)

        # Randomly select  two folds
        test_idx = dataset_idx[0:4]
        ...
        test_idxs = [idx for idx in range(len(test_dataset))]

        # Randomly select 3 folds for validation
        val_idx = dataset_idx[4:]

        val_dataset = ConcatDataset([fold_datasets[idx] for idx in val_idx])
        for dataset in val_dataset.datasets:
            dataset.transform = transform_chain_basic
        
        # Train dataset
        train_idx = np.ones((cfg.numFolds,),dtype=int) # Train fold number
        # Set the test and val folds to 0
