...
if __name__ == '__main__':
    ...
    results_path = results_path + '/' + eConfig['dir_weights_file'] + "_5fold_70train"
    if not os.path.exists(results_path):
        os.makedirs(results_path)
    ...
    scores1_matrix = np.full((len(full_dataset),), np.nan)
    scores2_matrix = np.full((len(full_dataset),), np.nan)
    ...
    torch.cuda.manual_seed(int(eConfig['seed']))
    torch.cuda.manual_seed_all(int(eConfig['seed']))
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False  # Asegura determinismo

    # iterations loop
    for fold in np.arange(cfg.numFolds):
        ...
        # AUCs
        AUCs[fold] = roc_auc_score(test_labels.flatten(), test_scores.flatten())
        if int(eConfig['type']) == 2:
            AUCs_out1[fold] = roc_auc_score(test_labels.flatten(), test1_scores.flatten()) 
            AUCs_out2[fold] = roc_auc_score(test_labels.flatten(), test2_scores.flatten())     

        # Logits matriz for each id
        scores_matrix[test_idxs] = test_scores
        scores1_matrix[test_idxs] = test1_scores
        scores2_matrix[test_idxs] = test2_scores
    ...
    # Show results
    print('mean testAUC accross folds: %f \n'%(np.mean(AUCs)))
    print('mean testAUC accross folds (out1): %f \n'%(np.mean(AUCs_out1)))
    print('mean testAUC accross folds (out2): %f \n'%(np.mean(AUCs_out2)))

    with open(text_file, "a") as file:
            # Escribe en el file
            file.write('\n mean testAUC accross folds: %f \n'%(np.mean(AUCs)))
            file.write('\n mean testAUC accross folds (out1): %f \n'%(np.mean(AUCs_out1)))
            file.write('\n mean testAUC accross folds (out2): %f \n'%(np.mean(AUCs_out2)))

    # Save results
    save_results_path = results_path + '/results_nn_5folds'
