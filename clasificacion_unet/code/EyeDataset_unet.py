...
class EyeDataset(Dataset):
  ...
  def __getitem__(self, idx): # carga imagen y su mask, seg y labels y lo recoge todo en sample (que es un dict)  
      ...
      sample = {'id': self.dataset.iloc[idx]['img_id'],'image': image, 'label':float(self.dataset.iloc[idx]['label']), 'img_path': self.dataset.iloc[idx]['img_path']}
      if self.transform:
           sample = self.transform(sample)

      # #Add num_frames
      # sample['numFrames'] = num_frames
      # sample['idx'] = idx
