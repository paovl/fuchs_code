...
if __name__ == '__main__':
    ...
    normalization_file = os.path.join(db_path,'dataset_global/SEP/ransac_TH_1.5_r_45/data/normalization.npz')
    ...
    transform_chain = transforms.Compose([cropEye(mapList, border=10),
                                            reSize(imSize),
                                            PolarCoordinates(max_radius=imSize[0]/2, max_degrees=360), ToTensor(), Normalize(mean=mean, std=std)])
    
    full_dataset = EyeDataset(csvFile, dataDir, transform = transform_chain, mapList = mapList)
