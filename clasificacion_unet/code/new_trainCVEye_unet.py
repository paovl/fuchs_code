...
import numpy as np
import pickle
from new_dataAugmentation_unet import Normalize, ToTensor, RandomRotation,RandomTranslation,centerMask,centerMinPAC, cropEye, reSize, PolarCoordinates, centerCrop, RandomJitter, SSL_augmentation
import time
import copy
...
if __name__ == '__main__':
    ...
    results_path = results_path + '/' + eConfig['dir_weights_file'] + "_prob" + str(eConfig['prob_aug'])
    if not os.path.exists(results_path):
        os.makedirs(results_path)
    ...
    if (eConfig['dir_weights_file'] != 'none') and ((eConfig['weights_seed'] != '-1') and (eConfig['weights_dseed'] != '-1')):
        if len(cfg.mapList) == 14: 
            weights_ssl = torch.load(path_weights_file + eConfig['dir_weights_file']+ '/allMaps/' + eConfig['weights_ssl_file'])
        elif len(cfg.mapList) == 1:
            weights_ssl = torch.load(path_weights_file + eConfig['dir_weights_file']+ '/' + cfg.mapList[0] + '/' + eConfig['weights_ssl_file'])
        else:
            weights_ssl = torch.load(path_weights_file + eConfig['dir_weights_file']+ '/' + '_'.join(cfg.mapList) + '/' + eConfig['weights_ssl_file'])
    else:
        weights_ssl = None
    ...
    # Load weights
    if weights_ssl is not None:
        model_ssl.load_state_dict(weights_ssl)
    ...
    prob_aug = float(eConfig['prob_aug'])
    if prob_aug > 0:
        list_transforms = list_transforms + [SSL_augmentation(model_ssl, mapList = cfg.mapList, prob = prob_aug)]
    ...
    for iter in np.arange(iterations):
        ...
        model_ft = UNetModel(nMaps, 1, weights_ssl)
