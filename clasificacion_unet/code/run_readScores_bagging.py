"""
@author: pvltarife
"""
import os
import subprocess
import random

def run_script(script, parameters):
    try:
        process = subprocess.run(['python', script] + parameters, check=True)
        print(f"-----------------RUNNING SCRIPT----------------------")
        # Check if the process ended correctly
        if process.returncode == 0:
            print(f"The running of {script} has ended successfully.")
        else:
            print(f"The running of {script} has ended because an error ocurred.")
    except subprocess.CalledProcessError as e:
        print("Error at running {script}: {e}")

if '___main___':

    script = "readScores_bagging.py"
    
    # dir = "none"
    # print(script + ' data -------> ' + dir)
    # parameters = ["dir_results", dir]
    # run_script(script, parameters)

    # Experiments unet

    seed = 0
    dseed = 0

    # patch_type = 'arc'
    # patch_num = [1]
    # loss = ['MSE']
    # dseed = 0
    # seed = 0
    # dilation = [0]

    # for i in patch_num:
    #     for l in loss:
    #         for d in dilation:
    #             dir = "unet_" + patch_type + "_volume_seed" + str(seed) + "_dseed" + str(dseed) + "_num" + str(i) + "_" + l + "Loss(mask_dilation" + str(d) + ")_final"
    #             print(script + ' data -------> ' + dir)
    #             parameters = ["dir", dir]
    #             run_script(script, parameters)

    # # Rings
    # patch_size = [5, 10, 20]
    # patch_volume = ['False', 'True']
    # patch_type = 'ring'
    # patch_num = [1,3, 5]
    # patch_w_start = [40, 60, 80]

    # for i in patch_size:
    #     for j in patch_volume:
    #         for k in patch_num:
    #             for x in patch_w_start:
    #                 if j == 'False':
    #                     dir = "unet_" + patch_type + "_seed" + str(seed) + "_dseed" + str(dseed) + "_size" + str(i) + "_num" + str(k) + "_limit" + str(x) + "_MSELoss"
    #                 else:
    #                     dir = "unet_" + patch_type + "_volume" + "_seed" + str(seed) + "_dseed" + str(dseed) + "_size" + str(i) + "_num" + str(k) + "_limit" + str(x) + "_MSELoss"
    #                 print(script + ' data -------> ' + dir)
    #                 parameters = ["dir_results", dir]
    #                 run_script(script, parameters)
    
    # # Arcs

    # patch_size = [10, 20, 30]
    # patch_volume = ['False', 'True']
    # patch_type = 'arc'
    # patch_num = [1, 3, 5]
    # patch_h_start = [352]

    # for i in patch_size:
    #     for j in patch_volume:
    #         for k in patch_num:
    #             for x in patch_h_start:
    #                 if j == 'False':
    #                     dir = "unet_" + patch_type + "_seed" + str(seed) + "_dseed" + str(dseed) + "_size" + str(i) + "_num" + str(k) + "_limit" + str(x) + "_MSELoss"
    #                 else:
    #                     dir = "unet_" + patch_type + "_volume" + "_seed" + str(seed) + "_dseed" + str(dseed) + "_size" + str(i) + "_num" + str(k) + "_limit" + str(x) + "_MSELoss"
    #                 print(script + ' data -------> ' + dir)
    #                 parameters = ["dir_results", dir]
    #                 run_script(script, parameters)
    

    # # Both

    # patch_size = [30, 40, 50, 60]
    # patch_volume = ['False', 'True']
    # patch_type = 'both'
    # patch_num = [1, 3, 5]
    # patch_h_start = [352]
    # patch_w_start = [60, 80]

    # for i in patch_size:
    #     for j in patch_volume:
    #         for k in patch_num:
    #             for x in patch_w_start:
    #                 for y in patch_h_start:

    #                     if j == 'False':
    #                         dir = "unet_" + patch_type + "_seed" + str(seed) + "_dseed" + str(dseed) + "_size" + str(i) + "_num" + str(k) + "_limitw" + str(x) + "_h" + str(y) + "_MSELoss"
    #                     else:0, 10, 20, 30, 40r = "unet_" + patch_type + "_volume" + "_seed" + str(seed) + "_dseed" + str(dseed) + "_size" + str(i) + "_num" + str(k) + "_limitw" + str(x) + "_h" + str(y) + "_MSELoss"
                    
    #                     print(script + ' data -------> ' + dir)
    #                     parameters = ["dir_results", dir]
    #                     run_script(script, parameters)

    # Map

    # patch_type = 'map'
    # patch_num = [1]
    # loss = ['SSIM']

    # for i in patch_num:
    #     for l in loss:
    #         dir = "unet_" + patch_type + "_seed" + str(seed) + "_dseed" + str(dseed) + "_num" + str(i) + "_" + l + "Loss(mask)_prueba"
    #         print(script + ' data -------> ' + dir)
    #         parameters = ["dir_results", dir]
    #         run_script(script, parameters)

    # patch_type = 'both'
    # patch_num = [1]
    # loss = ['MSESSIM']
    # alpha = [0.6, 0.5, 0.7,0.8, 0.9, 0.4, 0.3, 0.2, 0.1, 1, 0]
    # dilation = [3]

    # for i in patch_num:
    #     for l in loss:
    #         for d in dilation:
    #             for a in alpha:
    #                 dir = "unet_" + patch_type + "_seed" + str(seed) + "_dseed" + str(dseed) + "_num" + str(i) + "_" + l + "Loss_alpha" + str(a) + "(mask_dilation" + str(d) + ")_prueba"
    #                 print(script + ' data -------> ' + dir)
    #                 parameters = ["dir_results", dir]
    #                 run_script(script, parameters)

    # patch_type = 'both'
    # patch_num = [1]
    # loss = ['MSE']
    # dilation = [3]

    # for i in patch_num:
    #     for l in loss:
    #         for d in dilation:
    #             dir = "unet_" + patch_type + "_seed" + str(seed) + "_dseed" + str(dseed) + "_num" + str(i) + "_" + l + "Loss(mask_dilation" + str(d) + ")_prueba"
    #             print(script + ' data -------> ' + dir)
    #             parameters = ["dir_results", dir]
    #             run_script(script, parameters)
    
    # patch_type = 'map'
    # patch_num = [2]
    # loss = ['MSE']
    # dilation = [3]

    # for i in patch_num:
    #     for l in loss:
    #         for d in dilation:
    #             dir = "unet_" + patch_type + "_seed" + str(seed) + "_dseed" + str(dseed) + "_num" + str(i) + "_" + l + "Loss(mask_dilation" + str(d) + ")_prueba"
    #             print(script + ' data -------> ' + dir)
    #             parameters = ["dir_results", dir]
    #             run_script(script, parameters)
        
    # patch_type = 'arc'
    # patch_num = [1]
    # loss = ['MSE']
    # dilation = [3]

    # for i in patch_num:
    #     for l in loss:
    #         for d in dilation:
    #             dir = "unet_" + patch_type + "_seed" + str(seed) + "_dseed" + str(dseed) + "_num" + str(i) + "_" + l + "Loss(mask_dilation" + str(d) + ")_prueba"
    #             print(script + ' data -------> ' + dir)
    #             parameters = ["dir_results", dir]
    #             run_script(script, parameters)
        

    # patch_type = 'ring'
    # patch_num = [1]
    # loss = ['MSE']
    # dilation = [3]

    # for i in patch_num:
    #     for l in loss:
    #         for d in dilation:
    #             dir = "unet_" + patch_type + "_seed" + str(seed) + "_dseed" + str(dseed) + "_num" + str(i) + "_" + l + "Loss(mask_dilation" + str(d) + ")_prueba"
    #             print(script + ' data -------> ' + dir)
    #             parameters = ["dir_results", dir]
    #             run_script(script, parameters)
        
    # patch_type = 'map'
    # patch_num = [1]
    # loss = ['SSIM']
    # erosion = [1, 3, 5, 7, 9]

    # for i in patch_num:
    #     for l in loss:
    #         for e in erosion:
    #             dir = "unet_" + patch_type + "_seed" + str(seed) + "_dseed" + str(dseed) + "_num" + str(i) + "_" + l + "Loss(mask_erosion" + str(e) + ")_prueba"
    #             print(script + ' data -------> ' + dir)
    #             parameters = ["dir_results", dir]
    #             run_script(script, parameters)



    # patch_type = 'map'
    # patch_num = [1]
    # loss = ['MSE']
    # dilation = [3]
    # prob = [0, 0.1, 0.2, 0.3]

    # for i in patch_num:
    #     for l in loss:
    #         for d in dilation:
    #             for p in prob:
    #                 dir = "unet_" + patch_type + "_seed" + str(seed) + "_dseed" + str(dseed) + "_num" + str(i) + "_" + l + "Loss(mask_dilation" + str(d) + ")_prob" + str(p)
    #                 print(script + ' data -------> ' + dir)
    #                 parameters = ["dir_results", dir]
    #                 run_script(script, parameters)