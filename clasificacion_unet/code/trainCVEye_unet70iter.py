...
from models_unet import UNetModel_PolarPool as UNetModel
...
def train_model(model, image_datasets, dataloaders,criterion, optimizer, scheduler, text_file, device, num_epochs=25,max_epochs_no_improvement=10,min_epochs=10):
...
if __name__ == '__main__':
    ...
    eConfig = {
        ...
        'weights_dseed': '-1',
        'max_angle': '360',
        'weights_ssl_file': 'unet_self_supervised'
        }
    ...
    for iter in np.arange(iterations):
        ...
        dataset_idx = random.sample(range(cfg.numFolds), 3)
        # Test dataset
        test_idx = dataset_idx[0:2]
        test_dataset = ConcatDataset([fold_datasets[idx] for idx in test_idx])
        for dataset in test_dataset.datasets:
            dataset.transform = transform_chain_basic # Basic transformations and NOT data augmentation
        test_idxs = [indices for dataset in test_dataset.datasets for indices in dataset.indices]

        # Validation dataset
        val_idx = dataset_idx[2]
        ...
        if (eConfig['dir_weights_file'] != 'none') and ((eConfig['weights_seed'] != '-1') and (eConfig['weights_dseed'] != '-1')):
            if len(cfg.mapList) == 14:
                model_ssl_path = path_weights_file + eConfig['dir_weights_file']+ '/allMaps' + "/weights/" + eConfig['weights_ssl_file'] + "_iter" + str(iter) + ".pth"
                model_ssl = torch.load(model_ssl_path) 
            elif len(cfg.mapList) == 1:
                model_ssl_path = path_weights_file + eConfig['dir_weights_file']+ '/' + cfg.mapList[0]  + "/weights/" +eConfig['weights_ssl_file'] + "_iter" + str(iter) + ".pth"
                model_ssl = torch.load(model_ssl_path)
            else:
                model_ssl_path = path_weights_file + eConfig['dir_weights_file']+ '/' + '_'.join(cfg.mapList) + "/weights/" + eConfig['weights_ssl_file'] + "_iter" + str(iter) + ".pth"
                model_ssl = torch.load(model_ssl_path)
        else:
            model_ssl = None
        ...
        model_ft = train_model(model_ft, image_datasets, dataloaders, criterion, optimizer_ft, exp_lr_scheduler, text_file,
                               device, num_epochs=cfg.num_epochs, max_epochs_no_improvement=cfg.max_epochs_no_improvement,min_epochs=10)
