...
def computeMaxMinBFS(data_path, dataset, fname, outname, rng,  th = 1.5, diskRadius= 45, ransac = 1, px = 100):
    ...
    for k in range(iterations):
        ...
        if ele and ransac != -1 and k == 4 :
            ...
            if ransac == 1:
                pixels, _ = ransac_bfs(pixels, rng, th = th, diskRadius = diskRadius, state = 1)
...
def dataGenerator(data_path, error_BFS_path, error_PAC_path, image_path, heatmap_path, polar_image_path, dataset,fname, outname, patient, rng, th = 1.5, diskRadius=45, ransac = 1, px=100):
    ...
    for k in range(iteracion):
        ...
        if ele and ransac!=-1 and k==4:
            ...
            if ransac == 1:
                pixels_rel_enhanced, error_BFS = ransac_bfs(pixels, rng, th = th, diskRadius= diskRadius, state = 1)
                if od:
    ...
    for patient in os.listdir(db_path):
        ...
        for f in csv_files:
            ...
            for map_cat in map_cats:
                ...
                print(outname)
                dataset = readCSV(patient_path,csv_name)
                dataGenerator(data_path, error_BFS_path, error_PAC_path, image_path, heatmap_path, polar_image_path, dataset,name, outname, patient, rng, th = float(eConfig['th']), diskRadius = int(eConfig['r']), ransac = int(eConfig['ransac']), px = int(eConfig['pixels']))
