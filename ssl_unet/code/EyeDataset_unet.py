...
class EyeDataset(Dataset):
    def __init__(self, csv_file, db_path, data_dir, transform=None, mapList=None, imSize=(141,141), error_type = ''):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(nMaps):
            ...
            if self.error_type == '': 
                ...
                cmap_error = np.load( self.db_path + '/' + 'dataset_' + db +  '_unet/' + self.data_dir + '_error('+ self.error_type + ')/' + impath)
                cmap_error = resize(cmap_error, self.imSize, preserve_range=True)
        ...
        img_info = {'idx': idx, 'patient':patient, 'eye': eye_type, 'session': session, 'img':image['image'], 'label': image['label'], 'temp': temp_patient, 'mask': image['mask'], 'error': image['error']}
        return img_info
