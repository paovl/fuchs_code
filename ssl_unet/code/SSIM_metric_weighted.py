...
class SSIM_metric(torch.nn.Module):
    def __init__(self, window_size = 7, k1 = 0.01, k2 = 0.03, data_range = 1, spatial_dims = 2, device = "cuda:0", reduction='mean', alpha_weighted=0.5):
    ...
    def forward(self, img1, mask, img2, error, normalizer):
        ...
        weighted_ssim_idx = map_mean_sample

        weighted_ssim_idx = weighted_ssim_idx * 10

        weighted_ssim_idx = (1 - weighted_ssim_idx)

        if self.reduction == 'mean':
            weighted_ssim_idx = weighted_ssim_idx.mean()
