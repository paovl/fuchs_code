...
def computeNorm(annFile, db, px = 100):
    ...
    imgs_path = np.array(db + '/' +'dataset_' + dataset['Database'].astype(str)+ "_unet" + '/data/' + dataset['Patient'].astype(str))
...
if __name__ == "__main__":
    ...
    db_root = '../datasets'
    db_paths = ['../datasets/dataset_ssl_unet', '../datasets/dataset_multicentrico_unet']
    out_db = '../datasets/dataset_global_unet'
    ...
    normalization_data = {'categories': list(cats), 'mean': mu, 'std': sigma}
