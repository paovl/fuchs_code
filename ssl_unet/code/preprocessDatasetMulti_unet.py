...
def computeMaxMinBFS(fpath, dataset, fname, outname, info_dataset, rng, th = 1.5, diskRadius= 45, ransac = 1, px = 100): 
    ...
    for k in range(iterations):
        ...
        if ele and ransac != -1 and (k == 4 or k == 3):
            ...
            if ransac == 1:
                pixels, _, error_BFS_map = ransac_bfs(pixels, rng, label, th = th, diskRadius= diskRadius, state = 1)
...
def dataGenerator(fpath, error_BFS_path, error_PAC_path, error_CD_path, ipath, hpath, ppath, dataset, fname, outname, info_dataset, rng,  th = 1.5, diskRadius= 45, ransac = 1, px = 100):

    global PAC_max, PAC_min, CUR_max, CUR_min, ELE_max, ELE_min, CORNEA_DENS_max, CORNEA_DENS_min, ELE_BFS_max, ELE_BFS_min,ELE_BFS_8mm_min, ELE_BFS_8mm_max, error_BFS_map_max, error_BFS_map_min, error_PAC_map_max, error_PAC_map_min, error_CD_map_max, error_CD_map_min
    ...
    for k in range(iterations):
        ...
        if pac:
            ...
            np.save(os.path.join(fpath + "_error",outname + '_' + str(k) + '.npy'), error_PAC_map)
            
            error_PAC_map_image = error_PAC_map.copy()
            error_PAC_map_image[~mask] = 0

            imageGenerator(ipath + "_error", error_PAC_map_image, outname + '_' + str(k) + '.png', label)
        ...
        if ele and ransac != -1 and (k == 4 or k == 3):
            ...
            # RANSAC algorithm with sphere fit
            if ransac == 1:
                pixels, error_BFS, error_BFS_map = ransac_bfs(pixels, rng, label, th = th, diskRadius= diskRadius)
                if od:
                ...
                np.save(os.path.join(fpath + "_error",outname + '_' + str(k) + '.npy'), error_BFS_map)
                
                error_BFS_map_image = error_BFS_map.copy()
                error_BFS_map_image[~mask] = 0
                imageGenerator(ipath + "_error", error_BFS_map_image, outname + '_' + str(k) + '.png', label)
        ...
        if cd:
            ...
            np.save(os.path.join(fpath + "_error",outname + '_' + str(k) + '.npy'), error_CD_map)
            
            error_CD_map_image = error_CD_map.copy()
            error_CD_map_image[~mask] = 0

            imageGenerator(ipath + "_error", error_CD_map_image, outname + "_" + str(k) + '.png', label)
...
def ransac_cd(cdMap, rng,  k=10, th = 0.5, diskRadius = 0, state = 0):
    ...
    G_bfs = poly_matrix(x_mm, y_mm, order = ordr) 
    
    z_bfs = np.dot(G_bfs, m_bfs)
...
def computeNorm(annFile, data_path, px = 100):
    ...
    listFiles=glob.glob(os.path.join(data_path,'9_*.npy'))
...
if __name__ == "__main__":
    ...
    db_path = '../datasets/dataset_multicentrico_unet'
    ...
    imageHealthy_error_path = image_error_path + '/healthy'
    imageSick_error_path = image_error_path + '/sick'
    imageNolabel_error_path = image_error_path + '/noLabel'
    ...
    if not os.path.exists(imageHealthy_error_path):
        os.makedirs(imageHealthy_error_path)

    if not os.path.exists(imageSick_error_path):
        os.makedirs(imageSick_error_path)

    if not os.path.exists(imageNolabel_error_path):
        os.makedirs(imageNolabel_error_path)
    ...
    for patient, hospital, label in zip(db_index['img_id'], db_index['hospital'], db_index['label']):
        ...
        for subfolder in ['BASAL', 'TRAS FACO']:
            folder = find_folder(base_folder, subfolder)

            if folder == None:
                continue

            csv_files = glob.glob(folder + '/*PAC.CSV')
            patients_path = [folder for file in csv_files]
            list_maps = glob.glob(folder + '/*.CSV')
            categories = [os.path.basename(fichero).split('_')[-1][:-4] for fichero in list_maps]

            if not set(map_cats).issubset(set(categories)):
                continue
    ...
    for i in np.arange(len(error_BFS_map_max)):
        print('error_BFS_map[{}]'.format(i))
        print('max = ' + str(error_BFS_map_max[i]))
        print('min = ' + str(error_BFS_map_min[i]))
    
    for i in np.arange(len(error_CD_map_max)):
        print('error_CD_map[{}]'.format(i))
        print('max = ' + str(error_CD_map_max[i]))
        print('min = ' + str(error_CD_map_min[i]))
    
    for i in np.arange(len(error_PAC_map_max)):
        print('error_PAC_map[{}]'.format(i))
        print('max = ' + str(error_PAC_map_max[i]))
        print('min = ' + str(error_PAC_map_min[i]))
        ...
        for subfolder in ['BASAL', 'TRAS FACO']:
            ...
            for f, patient_path in zip_longest(csv_files, patients_path):
                ...
                for map_cat in map_cats:
                    ...
                    dataset = readCSV(patient_path,csv_name)
                    print(outname)
                    dataGenerator(data_path, error_BFS_path, error_PAC_path, error_CD_path, image_path, heat_path, polar_path, dataset, outname_short, outname, info_dataset, rng, th = float(eConfig['th']), diskRadius= int(eConfig['r']), ransac = ransac, px = pixels)
