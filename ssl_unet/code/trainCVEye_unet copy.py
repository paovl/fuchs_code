...
# Train model 
def train_model(model, image_datasets, mapList, dataloaders, criterion, optimizer, scheduler, device, results_path, normalizer, num_epochs=25, max_epochs_no_improvement=10, min_epochs=10, best_mse_model = 0, grad_cam_status = False, plot_image_best = [], save_plot_best = [], plot_batch_image_best = [], save_plot_batch_best = [], plot_results = [], save_plot_results = [], masked_imgs = [], output_imgs = [], imgs = [], residuals = [], patients_val_batch = []):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for img in dataloaders[phase]:
                ...
                outputs_m.extend(output.detach().cpu().numpy())

                ground_truth_m.extend(ground_truth.cpu().numpy())

                outputs_masked_m.extend(output[mask].detach().cpu().numpy())

                ground_truth_masked_m.extend(ground_truth[mask].cpu().numpy())

                mask_m.extend(mask.cpu().numpy())
            ...
            epoch_r2 = r2_score(torch.tensor(outputs_masked_m), torch.tensor(ground_truth_masked_m))
            # epoch_mse = mean_squared_error(np.array(outputs_masked_m), np.array(ground_truth_masked_m)).item()
            ...
            ground_truth_m = np.array(ground_truth_m, dtype=int)
            ...
            if phase == 'val' and epoch >= min_epochs:
                if epoch_mse < best_mse and epoch_loss < best_loss:  # or (epoch_auc == 1.0 and epoch_loss < best_loss):
                    best_mse = epoch_mse
                    ...
                    if best_mse < best_mse_model:
                        ...
                        if grad_cam_status == True and phase == 'val':
                            ...
                            for i in np.arange(img.shape[0]):
                                plot_image = save_gradcam_images({'image':img_copy[i,:,:,:], 'mask': mask[i]}, heatmaps[i,:,:], mapList, {key: value[i] for key, value in img_info.items()}, normalizer)
                                plot_result, masked_img, output_img, img_original = save_output_images({'image': label_copy[i,:,:,:], 'mask': mask[i]}, output[i,:,:], mapList, {key: value[i] for key, value in img_info.items()}, normalizer)
                                plot_array_result.append(plot_result)
                                plot_array.append(plot_image)
                                save_plot_array.append(img_info['patient'][i]+ '_' + str(img_info['eye'][i]) + '_' + img_info['session'][i]+ '.png')
                            ...
                            # residuals_array = list( ( residuals_aux - np.min(residuals_aux) ) / (np.max(residuals_aux) - np.min(residuals_aux)) )
                            residuals_array = list(abs(residuals_aux))

                            for i in np.arange(img.shape[0]):
                                plot_batch_image = save_gradcam_mean_images({'image':img_copy_mean[i,:,:,:], 'mask': mask[i]}, heatmap_mean, mapList, {key: value[i] for key, value in img_info.items()}, normalizer)
                                plot_batch_array.append(plot_batch_image)
                                save_plot_batch_array.append(img_info['patient'][i] + '_' + str(img_info['eye'][i]) + '_' + img_info['session'][i] + '.png')

                        patients_val_batch = {'idx': img_info['idx'], 'patient':img_info['patient'], 'loss': epoch_loss, 'output': outputs_m, 'ground_truth': ground_truth_m}
    ...
    print('Best model in epoch {:02d} val MSE {:4f}'.format(best_epoch, best_mse))

    with open(text_file, "a") as file:
        file.write('Training complete in {:.0f}m {:.0f}s\n'.format(
            time_elapsed // 60, time_elapsed % 60))
        file.write('Best model in epoch {:02d} val MSE {:4f}\n'.format(best_epoch, best_mse))

    # load best model weights
    model.load_state_dict(best_model_wts)
    return model, best_mse_model, plot_image_best, save_plot_best, plot_batch_image_best, save_plot_batch_best, plot_results, save_plot_results, masked_imgs, output_imgs, imgs, residuals, patients_val_batch
...
def save_output_images(img, output, mapList, img_info, normalizer):
    mask = img['mask']
    ...
    fig, axs = plt.subplots(img.shape[0], 3, figsize=(25, 10))  # 2 filas (una por imagen), 4 columnas (una por canal)
    ...
    for i in range(img.shape[-1]):
        ...
        masked_img_channel = (masked_img[ :, :, i] * 255).astype(np.uint8)

        axs[i, idx].imshow(masked_img_channel)
        axs[i, idx].axis('off')
        axs[i, idx].set_title(mapList[i])
        axs[i, idx].set_title(f'Input - ' + mapList[i])

        idx = idx + 1

        output_channel = (output[ :, :, i] * 255).astype(np.uint8)
        axs[i, idx].imshow(output_channel)
        axs[i, idx].axis('off')
        axs[i, idx].set_title(f'Output - ' + mapList[i])

        idx = idx + 1

        img_channel = (img[ :, :, i] * 255).astype(np.uint8)

        axs[i, idx].imshow(img_channel)
        axs[i, idx].axis('off')
        axs[i, idx].set_title(mapList[i])
        axs[i, idx].set_title(f'Original - ' + mapList[i])
    ...
    plt.close(fig)
    return data, masked_img, output, img
...
if __name__ == '__main__':
    ...
    # Patch Hiding
    if cfg.patch_hiding:
        list_transforms = list_transforms + [PatchHide(int(eConfig['patch_size']), rng, volume = (eConfig['patch_volume'].lower() == 'true'), type = eConfig['patch_type'], patch_num = int(eConfig['patch_num']), w_start_limit = int(eConfig['patch_w_start']), h_start_limit = int(eConfig['patch_h_start']))]
    ...
    model_ft, best_mse_model, plot_image_best, save_plot_best, plot_batch_image_best, save_plot_batch_best, plot_results, save_plot_results, masked_imgs, output_imgs, imgs, residuals, patients_val_batch = train_model(model_ft, image_datasets, cfg.mapList, dataloaders, criterion, optimizer_ft, exp_lr_scheduler,
                            device, results_path, normalizer, num_epochs=cfg.num_epochs, max_epochs_no_improvement=cfg.max_epochs_no_improvement,min_epochs=5, best_mse_model = best_mse_model, grad_cam_status = cfg.grad_cam, plot_image_best = plot_image_best, save_plot_best=save_plot_best, plot_batch_image_best = plot_batch_image_best, save_plot_batch_best = save_plot_batch_best, plot_results= plot_results, save_plot_results = save_plot_results, masked_imgs = masked_imgs, output_imgs= output_imgs, imgs = imgs, residuals = residuals, patients_val_batch = patients_val_batch)
    ...
    for idx, image_data in enumerate(plot_results):
        plt.imshow(image_data)
        plt.title(save_plot_results[idx].replace('.png', ''))
        plt.axis('off')
    ...
    print("Best MSE model saved: ", best_mse_model)
    with open(text_file, "a") as file:
            file.write("Best MSE model saved: " + str(best_mse_model))
