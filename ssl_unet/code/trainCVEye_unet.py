...
def train_model(model, image_datasets, mapList, dataloaders, criterion, optimizer, scheduler, device, results_path, normalizer, loss_name, num_epochs=25, max_epochs_no_improvement=10, min_epochs=10, best_metric_model = 0, grad_cam_status = False, plot_results = [], save_plot_results = [], save_plot_results_title = [], masked_imgs = [], output_imgs = [], imgs = [], residuals = [], patients_val_batch = []):
    ...
    loss_values_train = []
    loss_values_val = []
    ssim_metric = SSIM_metric(reduction='none')
    mse_metric = MSE_metric(reduction='none')
    mse_loss = MSELoss()
    ssim_loss = SSIMLoss()
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for img in dataloaders[phase]:
                ...
                mse_running_loss += ((mse_loss(output, mask, label, normalizer))[0]* batchsize).item()
                ssim_running_loss += ((ssim_loss(output, mask, label, normalizer))[0] * batchsize).item()
                ...
                if loss_name == 'MSE' or loss_name == 'MAE':
                    ...
                    outputs_extra_m.extend(output.detach())

                    ground_truth_extra_m.extend(ground_truth.detach()) 

                else:
                    outputs_m.extend(output.detach())

                    ground_truth_m.extend(ground_truth.detach())
            ...
            if loss_name == 'MSE' or loss_name == 'MAE':
                epoch_metric = r2_score(np.array(outputs_masked_m), np.array(ground_truth_masked_m))

            elif loss_name == 'SSIM':
                epoch_metric, _, _ = ssim(torch.stack(outputs_m), torch.stack(mask_m), torch.stack(ground_truth_m), normalizer)
                ...
                epoch_ssim, _, _ = ssim(torch.stack(outputs_extra_m), torch.stack(mask_m), torch.stack(ground_truth_extra_m), normalizer)
                epoch_metric = criterion.alpha * epoch_r2 + (1 - criterion.alpha) * epoch_ssim 

            r2_scores = r2_score(np.array(outputs_masked_m).T, np.array(ground_truth_masked_m).T, multioutput='raw_values')
            # mse_errors, _, _ = mse_loss(torch.stack(outputs_m), torch.stack(mask_m).cpu(), torch.stack(ground_truth_m), normalizer)
            ...
            if loss_name == 'MSE' or loss_name == 'MAE' or loss_name == 'MSESSIM':
                ssim_scores, _, _ = ssim_metric(torch.stack(outputs_m), torch.stack(mask_m).cpu(), torch.stack(ground_truth_m), normalizer)
            else: 
                ssim_scores, _, _ = ssim_metric(torch.stack(outputs_m), torch.stack(mask_m), torch.stack(ground_truth_m), normalizer)
            ssim_scores = torch.mean(ssim_scores, dim=1)
            ...
            if phase == 'val' and epoch >= min_epochs:
                if epoch_metric > best_metric and epoch_loss < best_loss:  # or (epoch_auc == 1.0 and epoch_loss < best_loss):
                    ...
                    if best_metric > best_metric_model:
                        ...
                        patients_val_batch = {'idx': img_info['idx'], 'patient':img_info['patient'], 'r2_scores': r2_scores, 'mse_errors': mse_errors, 'ssim_errors': ssim_errors, 'output': outputs_m, 'ground_truth': ground_truth_m}
...
def save_output_images(img, output,mapList, img_info, r2_score, normalizer):
    ...
    selected_ncc_map = calculate_ncc_map(selected_img, selected_output)
    ...
    # plot
    fig, axs = plt.subplots(selected_img.shape[2], 4, figsize=(25, 10))  # 2 filas (una por imagen), 4 columnas (una por canal)
    ...
    if selected_img.shape[2] == 1:
        ...
        axs[idx].imshow(selected_ncc_map_channel, cmap='gray', vmin=0, vmax=255)
        axs[idx].axis('off')
        ...
        for i in range(selected_img.shape[2]):
            ...
            axs[i, idx].imshow(selected_ncc_map_channel, cmap='gray', vmin=0, vmax=255)
            axs[i, idx].axis('off')
