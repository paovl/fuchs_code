    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if loss_name == 'MSE' or loss_name == 'MAE':
                epoch_metric = r2_score(np.array(outputs_masked_m), np.array(ground_truth_masked_m), mask= np.array(mask_masked_m) * ( 1 + criterion.alpha_weighted * np.array(error_masked_m)))
                ...
                epoch_r2 = r2_score(np.array(outputs_masked_m), np.array(ground_truth_masked_m), mask= np.array(mask_masked_m) * ( 1 + criterion.alpha_weighted * np.array(error_masked_m)))
            ...
            r2_scores = r2_score(np.array(outputs_masked_m), np.array(ground_truth_masked_m), mask= np.array(mask_masked_m) * ( 1 + criterion.alpha_weighted * np.array(error_masked_m)), multioutput='raw_values')
